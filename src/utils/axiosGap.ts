...
function getCombinedCa(): string {
  if (cachedCa) return cachedCa;

  // Load GAP CA certificate
  let gapCaCert: string;
  try {
    gapCaCert = fs.readFileSync(getGapCaPath(), 'utf-8');
  } catch (err) {
    throw new Error(
      `Failed to load GAP CA certificate from ${getGapCaPath()}. ` +
 ...
 */
function getGapToken(providedToken?: string): string {
  const token = providedToken || process.env.GAP_TOKEN;

  if (!token) {
    throw new Error(
      'GAP_TOKEN not found. Please set GAP_TOKEN in your .env file or environment.\n' +
 ...
 */
export function create(options?: CreateAxiosGapOptions): AxiosInstance {
  ...
  agent.connect = function(req: any, opts: any) {
    opts.ca = combinedCa;
